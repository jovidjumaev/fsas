...
async function getStudentAttendanceData(classId, studentName = null) {
  try {
    logger.log('📊 Fetching student attendance data for class:', classId);
    logger.log('🔍 Looking for student:', studentName);
    logger.log('🔄 Cache-busting timestamp:', Date.now());
    ...
    logger.log('📋 Found enrollments:', enrollments?.length || 0);
    
    if (!enrollments || enrollments.length === 0) {
      logger.log('❌ No enrollments found for class:', classId);
      return [];
    }
    ...
    logger.log('👥 Found students:', students?.length || 0);
    ...
    if (studentName) {
      ...
      logger.log('🔍 Filtered students for', studentName, ':', filteredStudents.length);
    }
    ...
    logger.log('📅 Found sessions:', sessions?.length || 0);
    ...
    logger.log('✅ Student attendance data fetched:', processedData.length, 'students');
    return processedData;
    
  } catch (error) {
    logger.error('❌ Error in getStudentAttendanceData:', error);
    return null;
  }
}
...
async function getClassOverviewData(classId) {
  try {
    ...
    if (error) {
      logger.error('❌ Error fetching class overview data:', error);
      return null;
    }
    ...
    logger.log('✅ Class overview data fetched');
    return processedData;
    
  } catch (error) {
    logger.error('❌ Error in getClassOverviewData:', error);
    return null;
  }
}
...
function extractStudentNameFromQuestion(question) {
  ...
  for (const generalQ of generalQuestions) {
    if (lowerQuestion.includes(generalQ)) {
      logger.log('🔍 General question detected, returning null for ALL students');
      return null;
    }
  }
  ...
  logger.log('🔍 No specific student name found, returning null for ALL students');
  return null;
}
  ...
  try {
    ...
    try {
      if (req.file.mimetype === 'application/pdf') {
        ...
        logger.log('📊 Processing PowerPoint file:', req.file.originalname);
        logger.log('📊 File size:', req.file.size, 'bytes');
        logger.log('📊 MIME type:', req.file.mimetype);
        ...
        try {
          ...
          logger.log('✅ PowerPoint text extracted:', extractedText.length, 'characters');
        } catch (pptxError) {
          logger.error('❌ PowerPoint extraction error:', pptxError);
        ...
        logger.log('📊 Processing legacy PowerPoint file:', req.file.originalname);
        logger.log('📊 File size:', req.file.size, 'bytes');
        logger.log('📊 MIME type:', req.file.mimetype);
        ...
        logger.log('✅ Legacy PowerPoint file processed successfully');
      }
    } catch (extractError) {
    ...
    // Save to database
    logger.log('💾 Saving material to database:', {
    ...
    logger.log('✅ Material saved successfully:', materialData.id);
...
router.get('/api/classes/:classId/materials', async (req, res) => {
  try {
    ...
    logger.error('❌ Error in GET materials:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
router.delete('/api/classes/:classId/materials/:materialId', async (req, res) => {
  try {
    ...
    logger.log('🗑️ Delete request received:', { classId, materialId, professorId });
    ...
    if (classError || !classInstance) {
      logger.log('❌ Access denied:', classError);
      return res.status(403).json({
        success: false,
        error: 'Access denied to this class'
      });
    ...
    if (materialError || !material) {
      logger.log('❌ Material not found:', materialError);
    ...
    logger.log('📁 Found material to delete:', material.file_name);
    logger.log('🔗 File URL:', material.file_url);
    logger.log('📄 File type:', material.file_type);
    ...
    if (isPowerPoint) {
      logger.log('📊 This is a PowerPoint file - enhanced logging enabled');
    }
    
    // List files in storage bucket to see what's actually there
    logger.log('📋 Listing files in storage bucket...');
    ...
    if (listError) {
      ...
      logger.log('📋 Files in bucket:', bucketFiles?.map(f => f.name) || []);
      ...
      if (isPowerPoint) {
        ...
        logger.log('📊 PowerPoint files that might match:', matchingFiles.map(f => f.name));
      }
    }
    ...
    try {
      ...
      if (isPowerPoint) {
        logger.log('📊 PowerPoint filename extraction details:');
        logger.log('📊 Original file_name:', material.file_name);
        logger.log('📊 Extracted from URL:', fileName);
        logger.log('📊 URL parts:', urlParts);
        ...
        if (bucketFiles) {
          ...
          logger.log('📊 Exact match found:', exactMatch?.name || 'none');
          logger.log('📊 Name match found:', nameMatch?.name || 'none');
          logger.log('📊 Partial match found:', partialMatch?.name || 'none');
          
          // Use the best match
          if (exactMatch) {
            fileName = exactMatch.name;
            logger.log('📊 Using exact match:', fileName);
          } else if (nameMatch) {
            fileName = nameMatch.name;
            logger.log('📊 Using name match:', fileName);
          } else if (partialMatch) {
            fileName = partialMatch.name;
            logger.log('📊 Using partial match:', fileName);
          }
        }
      }
    } catch (error) {
    ...
    logger.log('🗑️ Attempting to delete from storage bucket "class-materials"');
    logger.log('📄 Filename to delete:', fileName);
    logger.log('🔗 Original file URL:', material.file_url);
    
    const { data: storageData, error: storageError } = await supabase.storage
      .from('class-materials')
      .remove([fileName]);

    logger.log('🗑️ Storage deletion result:', { storageData, storageError });

    if (storageError) {
      logger.error('❌ Storage deletion error:', storageError);
      logger.log('📄 Attempted filename:', fileName);
      logger.log('🔗 Original URL:', material.file_url);
      
      // Try alternative filename extraction
      logger.log('🔄 Trying alternative filename extraction...');
      const alternativeFileName = material.file_name;
      logger.log('📄 Alternative filename:', alternativeFileName);
      
      const { data: altStorageData, error: altStorageError } = await supabase.storage
        .from('class-materials')
        .remove([alternativeFileName]);
      
      logger.log('🗑️ Alternative storage deletion result:', { altStorageData, altStorageError });
      
      if (altStorageError) {
        logger.error('❌ Alternative storage deletion also failed:', altStorageError);
        
        // For PowerPoint files, try additional approaches
        if (isPowerPoint && bucketFiles) {
          logger.log('📊 PowerPoint deletion failed - trying all possible filenames...');
          ...
          logger.log('📊 Possible PowerPoint files to delete:', possibleFiles.map(f => f.name));
          
          if (possibleFiles.length > 0) {
            const fileNamesToDelete = possibleFiles.map(f => f.name);
            logger.log('📊 Attempting to delete PowerPoint files:', fileNamesToDelete);
            
            const { data: pptStorageData, error: pptStorageError } = await supabase.storage
              .from('class-materials')
              .remove(fileNamesToDelete);
            
            logger.log('🗑️ PowerPoint storage deletion result:', { pptStorageData, pptStorageError });
            
            if (!pptStorageError) {
              logger.log('✅ PowerPoint files deleted from storage:', fileNamesToDelete);
            } else {
              logger.error('❌ PowerPoint storage deletion failed:', pptStorageError);
            }
          }
        }
      } else {
        logger.log('✅ File deleted from storage with alternative filename:', alternativeFileName);
      }
    } else {
      logger.log('✅ File deleted from storage:', fileName);
      logger.log('📊 Storage deletion data:', storageData);
    }
    ...
    logger.log('✅ Material deleted from database');
...
router.post('/api/classes/:classId/chat/message', async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error('❌ Error fetching database context:', error);
    }
...
- Prioritize accuracy over verbosity`
    ...
    if (tokensUsed > 100) {
      logger.log(`⚠️ High token usage: ${tokensUsed} tokens for response`);
    }
    
    // Truncate response if it's too long (additional safety)
    if (aiResponse.length > 500) {
      finalResponse = aiResponse.substring(0, 500) + '...';
      logger.log('⚠️ Response truncated due to length');
    }
...
router.get('/api/debug/class/:classId/students', async (req, res) => {
  try {
    const { classId } = req.params;
    logger.log('🔍 DEBUG: Testing database queries for class:', classId);
    ...
    logger.log('🔍 DEBUG: Enrollments result:', { enrollments, enrollmentsError });
    ...
    logger.log('🔍 DEBUG: Students result:', { students, studentsError });
    ...
    logger.log('🔍 DEBUG: Sessions result:', { sessions, sessionsError });
    ...
    logger.error('❌ DEBUG: Error in debug endpoint:', error);
